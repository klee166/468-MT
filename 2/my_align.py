...
## using transition probabilities t(f|e) and most probable alignment.
...
e_count = defaultdict(float) # Maps an English word to expected count
fe_count = defaultdict(float) # Maps all (f,e) word pairs to expected count
t_probability = defaultdict(float) # transition probability of f->e
...
while k < 5:
    ...
    # Set count(f|e) = 0 for all (f,e) and set total(e) = 0 for all e
    for (n, (f, e)) in enumerate(bitext):
        for e_j in set(e):
            e_count[e_j] = 0
            for f_i in set(f):
    ...
    for (n, (f, e)) in enumerate(bitext):
        for (i, f_i) in enumerate(f):
            ...
            for (j, e_j) in enumerate(e):
                ...
                fe_count[(f_i, e_j)] += c # Increment count(f,e) by expected count
                e_count[e_j] += c # Increment count(e) by expected count
            if n % 5000 == 0:
                sys.stderr.write(".")
